...
uint8_t OldSpeedLeft=0;
uint8_t OldSpeedRight=0;
...
void MotorDir(uint8_t left, uint8_t right)
{
  ...
  if (left == BREAK)
  {
    digitalWrite(MOTOR_LEFT1, LOW);
    digitalWrite(MOTOR_LEFT2, LOW);
    MotorSpeed(255, OldSpeedRight);
  }
  if (left == FREE)
  {
    MotorSpeed(0, OldSpeedRight);
  }
  //********** right motor ********************
  if (right == FWD)
  {
  ...
  if (right == BREAK)
  {
    digitalWrite(MOTOR_RIGHT1, LOW);
    digitalWrite(MOTOR_RIGHT2, LOW);
    MotorSpeed(255, OldSpeedLeft);
  }
  if (right == FREE)
  {
    MotorSpeed(OldSpeedLeft, 0);
  }
